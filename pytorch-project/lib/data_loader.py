...
def create_data_loaders(
    ...
    features, targets, class_names = preprocessor.load_processed_data(data_path)
    ...
    # Persist fitted scaler next to the processed data for downstream use
    try:
        scaler_path = data_path + '.scaler.npz'
        preprocessor.save_scaler(scaler_path)
        logger.info(f'Saved fitted scaler to: {scaler_path}')
    except Exception as e:
        logger.warning(f'Could not save scaler to {data_path}.scaler.npz: {e}')

    # Save training data and validation data to text files
    # preprocessor.save_train_val_to_text(train_data, val_data)
